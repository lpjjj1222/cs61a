...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while beeplace and step < self.min_range or beeplace and not beeplace.bees: #如果Step还没到min_range或者step虽然大于min_range但是没有bees
            if beeplace.entrance == beehive:
                break
            else:
                if step < self.max_range: #如果Step还没到max_range
                    beeplace = beeplace.entrance
                    step += 1
                else: #如果step到了max_range，不能再走了
                    break

        return rANTdom_else_none(beeplace.bees)
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.authenticity == True:
            ThrowerAnt.action(self, gamestate)
            exit_p = self.place.exit
    
            while exit_p:
                if exit_p.ant and exit_p.ant.is_buffed == False:
                    exit_p.ant.damage = exit_p.ant.damage * 2
                    exit_p.ant.is_buffed = True
                else:
                    exit_p = exit_p.exit
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.authenticity == True:
            bees_win()
        # END Problem EC
